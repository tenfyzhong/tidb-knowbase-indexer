...
export function sanitizeLogs(config: Config, env: Env): void {
  ...
  if (env.CLOUDFLARE_API_TOKEN) secretsToMask.push(env.CLOUDFLARE_API_TOKEN);
  if (env.CLOUDFLARE_API_KEY) secretsToMask.push(env.CLOUDFLARE_API_KEY);
  if (env.CLOUDFLARE_ACCOUNT_ID) secretsToMask.push(env.CLOUDFLARE_ACCOUNT_ID);
